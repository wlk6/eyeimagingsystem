    ...
    <div>
      <vxe-grid ref="xGrid" 
        ...
        <template #avatarSlot="{ row }">
          <img :src="baseUrl + row.avatar" style="width: 50px; height: 48px; border-radius: 50%; object-fit: cover;" />
        </template>
      </vxe-grid>
    </div>
  </div>
...
export default {
  data() {
    return {
      baseUrl: 'http://121.196.228.176:9002/user', // 添加基础URL
      isCreating: false,
      tableData: [],
      ...
      fullColumns: [
        ...
        { 
          ...
          align: 'center',
          formatter: ({ cellValue }) => {
            return POSITION_MAP[cellValue] || cellValue;
          }
        },
        { 
          field: 'avatar', 
      ...
      gridOptions: {
        ...
        proxyConfig: {
          ...
          ajax: {
            query: () => {
              return fetch('http://121.196.228.176:9002/user/adminGetCheckList/', {
                method: 'POST',
 ...
 handleSave() {
  ...
  try {
    const response = await fetch('http://121.196.228.176:9002/user/adminGiveCheckAdvice/', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'token': `${getToken()}` 
    ...
    const data = await response.json();
    if (response.ok && data.msg === '审核医生信息成功') {
